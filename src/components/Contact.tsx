    ...
    <section id="contact" className="py-12 sm:py-16 lg:py-20 bg-gradient-to-b from-gray-50 to-gray-100 dark:from-gray-900 dark:to-black">
      <div className="container mx-auto px-4 sm:px-6 lg:px-8">
        ...
        <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-4 gap-6">
          ...
          {/* Email */}
          <Card gradient="from-blue-50 to-blue-100" border="border-blue-200">
            ...
            <CardButton href="mailto:riteshnvisonex@gmail.com" color="bg-blue-500 hover:bg-blue-600">
              riteshnvisonex@gmail.com
            </CardButton>
          </Card>

          {/* WhatsApp */}
          <Card gradient="from-green-50 to-green-100" border="border-green-200">
            <IconBox color="bg-green-500"><WhatsAppIcon width={24} height={24} /></IconBox>
            <CardTitle>WhatsApp</CardTitle>
            <CardDesc>Quick messages</CardDesc>
            <CardButton href="https://wa.me/918660144040" color="bg-green-500 hover:bg-green-600">
              +91 8660144040
            </CardButton>
          </Card>

          {/* LinkedIn */}
          <Card gradient="from-indigo-50 to-indigo-100" border="border-indigo-200">
            <IconBox color="bg-indigo-500"><Linkedin className="text-white w-6 h-6" /></IconBox>
            <CardTitle>LinkedIn</CardTitle>
            <CardDesc>Professional network</CardDesc>
            <CardButton href="https://linkedin.com/in/ritesh-n-5113b328a" color="bg-indigo-500 hover:bg-indigo-600">
              Connect
            </CardButton>
          </Card>

          {/* GitHub */}
          <Card gradient="from-gray-50 to-gray-100" border="border-gray-300">
            <IconBox color="bg-gray-700"><Github className="text-white w-6 h-6" /></IconBox>
            <CardTitle>GitHub</CardTitle>
            <CardDesc>Explore my projects</CardDesc>
            <CardButton href="https://github.com/Ritesh771" color="bg-gray-700 hover:bg-gray-800">
              Explore
            </CardButton>
          </Card>
...
const Card = ({ children, gradient, border }: any) => (
  <div className={`rounded-2xl p-6 bg-gradient-to-br ${gradient} border ${border} shadow-md hover:shadow-xl transform hover:scale-[1.02] transition-all duration-300 backdrop-blur-sm`}>
    {children}
  </div>
);

const IconBox = ({ children, color }: any) => (
  <div className={`w-12 h-12 ${color} rounded-xl flex items-center justify-center mb-3 shadow-md`}>{children}</div>
);

const CardTitle = ({ children }: any) => <h3 className="text-lg font-bold text-gray-900 dark:text-white">{children}</h3>;
const CardDesc = ({ children }: any) => <p className="text-gray-600 dark:text-gray-300 mb-4 text-sm">{children}</p>;
const CardButton = ({ href, children, color }: any) => (
  <a href={href} target="_blank" rel="noopener noreferrer"
     className={`inline-block px-4 py-2 rounded-xl text-white font-medium transition-all duration-300 ${color}`}>
    {children}
  </a>
);
